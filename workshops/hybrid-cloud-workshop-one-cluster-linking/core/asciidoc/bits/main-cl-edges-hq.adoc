...
image::./2_mysql_source_connector.png[]
...
*Note*: if you are interested in how replicator works you can go to link:replicator-tocloud.adoc[Replicator from on-prem to Cloud] and execute replicator just for testing purposes.
...
As these Confluent Platform (HQ) and  Confluent Cloud Instances involved in this step are being shared by other users of the workshop, all the topics to be mirrored are together in on-premise (HQ) cluster with different dc* prefix name.
So it will be required just to create only one source initiated link for all users, and then each user would add their own topics to be mirrored executing another command.

So both halves of the cluster link: the first half on Confluent Cloud, the second half on Confluent Platform (HQ) will be created by the instructor or one of the users, but *just 1 person needs to execute the following command*.

*Note*: another approach could be pointing to your own Confluent Cloud cluster so in that case you will need to execute both halves to create your own link . (Link to how to create your own cluster)
...
----
curl --request POST \
  --url ''${CCLOUD_REST_ENDPOINT}'/kafka/v3/clusters/'${CCLOUD_CLUSTER_ID}'/links?link_name=clusterlink-to-cc' \
...
[source,subs="attributes"]
----
curl -s --request GET   --url ''${CCLOUD_REST_ENDPOINT}'/kafka/v3/clusters/'${CCLOUD_CLUSTER_ID}'/links/clusterlink-to-cc' --header 'Authorization: Basic '$ENCODED_API_KEY_SECRET''|jq .
----
...
----
{
  "kind": "KafkaLinkData",
  "metadata": {
    "self": "https://pkc-xxxx1.us-east-2.aws.confluent.cloud/kafka/v3/clusters/lkc-qrkqn2/links/clusterlink-to-cc"
  },
  "source_cluster_id": "xxxxxxxxxxxxxxxxxx",
  "destination_cluster_id": null,
  "link_name": "clusterlink-to-cc",
  "link_id": "xxxxxx-xxxx-xxxx-xxxx-xxxxxx",
  "cluster_link_id": "xxxxxxxxx",
  "topic_names": [],
  "link_error": "NO_ERROR",
...
----
curl --request POST \
  --url ''${HQ_EXT_IP}:8090'/kafka/v3/clusters/'${ONPREM_HQ_CLUSTER_ID}'/links?link_name=clusterlink-to-cc' \
  --header 'Content-Type: application/json' \
  --data '{
...
[source,subs="attributes"]
----
curl -s --request GET --url http://${HQ_EXT_IP}:8090/kafka/v3/clusters/${ONPREM_HQ_CLUSTER_ID}/links/clusterlink-to-cc|jq .
----
  ...
  "data": [
    {
      "kind": "KafkaLinkData",
      "metadata": {
        "self": "http://localhost:8090/kafka/v3/clusters/xxxxxxxxx/links/clusterlink-to-cc"
      },
      "source_cluster_id": null,
...
So if the "auto.create.mirror.topics.enable" and "auto.create.mirror.topics.filters" configs are not configured each user has to add their own topics.
...
----
curl --request POST \
  --url ''${CCLOUD_REST_ENDPOINT}'/kafka/v3/clusters/'${CCLOUD_CLUSTER_ID}'/links/clusterlink-to-cc/mirrors' \
  --header 'Authorization: Basic '$ENCODED_API_KEY_SECRET'' \
  --header 'Content-Type: application/json' \
  --data '{
...
*Note*: In case you created the link in your own Confluent Cloud Cluster you need to check if the messages are arriving in the Confluent Cloud UI.
...
*Note*: if you are using your own Confluent cloud cluster you must use your own ksqldb cluster in the cloud. (How to create Confluent Cloud ksqldb cluster)
...
*Note*: If you are using your own Confluent Cloud Cluster, please adapt the command to use your cluster information.
...
----
curl --request POST \
  --url ''http://${HQ_EXT_IP}:8090'/kafka/v3/clusters/'${ONPREM_HQ_CLUSTER_ID}'/links?link_name=clusterlink-cp-hq-to-cp-edge-{dc}' \
  --header 'Content-Type: application/json' \
  --data '{
...
[source,subs="attributes"]
----
curl -s --request GET   --url http://${HQ_EXT_IP}:8090/kafka/v3/clusters/${ONPREM_HQ_CLUSTER_ID}/links/clusterlink-cp-hq-to-cp-edge-{dc} | jq .
----
...
----
{
  "kind": "KafkaLinkData",
  "metadata": {
    "self": "http://${HQ_EXT_IP}:8090/kafka/v3/clusters/hXFos4xuT_KypQik_Hgyag/links/clusterlink-cp-hq-to-cp-edge-{dc}"
  },
  "source_cluster_id": "lkc-xxxxx",
  "destination_cluster_id": null,
  "link_name": "clusterlink-cp-hq-to-cp-edge-{dc}",
...
----
curl --request POST \
  --url ''http://${HQ_EXT_IP}:8090'/kafka/v3/clusters/'${ONPREM_HQ_CLUSTER_ID}'/links/clusterlink-cp-hq-to-cp-edge-{dc}/mirrors' \
  --header 'Content-Type: application/json' \
  --data '{
...
----
curl --request POST \
  --url 'http://localhost:8090/kafka/v3/clusters/'${ONPREM_CLUSTER_ID}'/links?link_name=clusterlink-cp-hq-to-cp-edge' \
  --header 'Content-Type: application/json' \
  --data '{
    "source_cluster_id": "'${ONPREM_HQ_CLUSTER_ID}'",
    "configs": [
...
[source,subs="attributes"]
----
curl -s --request GET   --url http://localhost:8090/kafka/v3/clusters/${ONPREM_CLUSTER_ID}/links/clusterlink-cp-hq-to-cp-edge | jq .
----
...
----
{
  "kind": "KafkaLinkData",
  "metadata": {
    "self": "http://localhost:8090/kafka/v3/clusters/xxxxxxxxxxx/links/clusterlink-cp-hq-to-cp-edge"
  },
  "source_cluster_id": "lkc-xxxxx",
  "destination_cluster_id": null,
  "link_name": "clusterlink-cp-hq-to-cp-edge",
  "link_id": "xxxxx-xxxx-xxxxx-xxxxx-xxxxx",
  "cluster_link_id": xxxxxxxxxxxx",
  "topic_names": []
}
...
----
curl --request POST \
  --url 'http://localhost:8090/kafka/v3/clusters/'${ONPREM_CLUSTER_ID}'/links/clusterlink-cp-hq-to-cp-edge/mirrors' \
  --header 'Content-Type: application/json' \
  --data '{
    "source_topic_name": "{dc}_out_of_stock_events",
    "configs": [
